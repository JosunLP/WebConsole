...
export interface IWorkerManager extends IEventEmitter {
  ...
  // Pool Management
  createPool(id: string, maxWorkers?: number, permissions?: IWorkerPermissions): Promise<IWorkerPool>;
  destroyPool(id: string): Promise<void>;
  getPool(id: string): IWorkerPool | undefined;
  listPools(): string[];
  
  // Task Execution - Simplified API
  runTask<T, R>(taskFunction: () => T, options?: Partial<IWorkerTask>): Promise<R>;
  runParallel<T>(taskFunction: () => T, options?: Partial<IWorkerTask>): Promise<T>;
  runParallelBatch<T>(taskFunctions: (() => T)[], options?: Partial<IWorkerTask>): Promise<T[]>;
