...
export class WorkerCommand extends BaseCommand {
  ...
  async execute(context: CommandContext): Promise<ExitCode> {
    ...
    if (positional.length === 0) {
      return this.outputError(context, "Subcommand required. Use: status, config, create, destroy");
    }
  ...
  private async showStatus(context: CommandContext, workerManager: IWorkerManager): Promise<ExitCode> {
    const workerStatus = workerManager.getWorkerStatus();
    const activeWorkerCount = workerManager.getActiveWorkerCount();

    await this.writeToStdout(context, this.colorize("🔧 Worker Manager Status\n", BaseCommand.COLORS.BOLD));
    await this.writeToStdout(context, `Total Active Workers: ${activeWorkerCount}\n`);
    await this.writeToStdout(context, `Worker Pools: ${workerStatus.length}\n\n`);

    await this.writeToStdout(context, this.colorize("Detailed Pool Status:\n", BaseCommand.COLORS.CYAN));
    ...
    for (const pool of workerStatus) {
      ...
      await this.writeToStdout(context, `  Queued Tasks: ${pool.queuedTasks}\n`);
      await this.writeToStdout(context, `  Completed: ${pool.completedTasks}\n`);
  ...
  private async showConfig(context: CommandContext, workerManager: IWorkerManager): Promise<ExitCode> {
    const permissions = workerManager.getDefaultPermissions();

    await this.writeToStdout(context, this.colorize("⚙️ Worker Configuration\n", BaseCommand.COLORS.BOLD));
    await this.writeToStdout(context, `VFS Access: ${permissions.vfsAccess ? 'Enabled' : 'Disabled'}\n`);
    await this.writeToStdout(context, `Network Access: ${permissions.networkAccess ? 'Enabled' : 'Disabled'}\n`);
    await this.writeToStdout(context, `Max Execution Time: ${permissions.maxExecutionTime}ms\n`);
    
    if (permissions.memoryLimit) {
      await this.writeToStdout(context, `Memory Limit: ${this.formatFileSize(permissions.memoryLimit)}\n`);
    }

    return ExitCode.SUCCESS;
  }

  private async createPool(context: CommandContext, workerManager: IWorkerManager, args: string[]): Promise<ExitCode> {
    if (args.length === 0) {
    ...
    try {
      ...
      await this.writeToStdout(context, `Worker pool '${poolId}' created with ${maxWorkers} max workers\n`);
      return ExitCode.SUCCESS;
    } catch (error) {
  ...
  private async destroyPool(context: CommandContext, workerManager: IWorkerManager, args: string[]): Promise<ExitCode> {
    if (args.length === 0) {
      return this.outputError(context, "Pool ID required");
    }
