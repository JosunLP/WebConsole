...
export class ThemeCommand extends BaseCommand {
  ...
  private async showCurrentTheme(context: CommandContext): Promise<void> {
    ...
    await this.writeToStdout(context, `Current theme: ${this.colorize(currentTheme, 'cyan')}\n`);
    await this.writeToStdout(context, `Theme mode: ${this.colorize(mode, 'yellow')}\n`);
  ...
  private async listAvailableThemes(context: CommandContext): Promise<void> {
    ...
    for (const theme of ThemeCommand.AVAILABLE_THEMES) {
      const indicator = theme === currentTheme ? this.colorize('●', 'green') : ' ';
      const themeName = theme === currentTheme ? this.colorize(theme, 'green') : theme;
  ...
  private async setTheme(context: CommandContext, themeName: string): Promise<ExitCode> {
    // Validate theme name
    if (!ThemeCommand.AVAILABLE_THEMES.includes(themeName)) {
      await this.writeToStderr(context,
  ...
  private async showThemePreview(context: CommandContext): Promise<void> {
    ...
    for (const color of colors) {
      ...
      await this.writeToStdout(context, `  ${coloredText.padEnd(20)} ${brightColoredText}\n`);
    }
  ...
  private async applyTheme(context: CommandContext, themeName: string): Promise<void> {
    // Set theme in state manager
  ...
  private emitThemeChangeEvent(context: CommandContext, themeName: string): void {
   ...
   */
  static async applyTheme(context: CommandContext, themeName: string): Promise<boolean> {
